...
adata ,pred_dsc= STMSGAL.train_STMSGAL(adata, alpha=alpha, pre_resolution=pre_resolution,
                              ...
                              cost_ssc_coef = cost_ssc_coef, category = category)
...
# 	values=pred_dsc.astype('U'),
# 	categories=natsorted(map(str, np.unique(pred_dsc))),
# )
...
parameter = f"alpha={alpha},n_epochs={n_epochs}," \
            f"reg_ssc={reg_ssc_coef}," \
            f"cost_ssc = {cost_ssc_coef}"
