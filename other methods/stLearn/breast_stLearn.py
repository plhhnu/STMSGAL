...
proj_list = ['V1.1 Mouse Brain Serial Section 1 (Sagittal-Anterior)']
for proj_idx in range(len(proj_list)):
    ...
    def res_search_fixed_clus(adata, fixed_clus_count, increment=0.002):
        '''
            ...
            return:
                resolution[int]
        '''
        for res in sorted(list(np.arange(1.4, 2.5, increment)), reverse=True):
    ...
    eval_resolution = res_search_fixed_clus(data, n_clusters)
    st.tl.clustering.louvain(data, resolution=eval_resolution,random_state=0)
    ...
    # data.filename = f'./h5ad/{category}/final_V1_Breast_Cancer_Block_A_Section_1_stLearn_cluster20,resolution{eval_resolution}_2.h5ad'
    # print(data.isbacked)
    ...
    parameter = f"stLearn_cluster{n_clusters},resolution{eval_resolution}"
    ...
    #
    # if not os.path.exists(f'./outputss/{category}'):
    #     os.makedirs(f'./outputss/{category}')


    plt.rcParams["figure.figsize"] = (5, 5)
    sc.pl.spatial(data, img_key="hires", color="louvain", size=1.5, show=False,title='stLearn')
    plt.savefig(f'./outputs/{category}/stLearn_atest.jpg',
                bbox_inches='tight', dpi=150)
    plt.savefig(f'./outputs/{category}/size=1.5_{parameter}.jpg',
                bbox_inches='tight', dpi=150)
    sc.pl.spatial(data, img_key="hires", color="louvain", size=1.2, show=False,title='stLearn')
    plt.savefig(f'./outputs/{category}/size=1.2_{parameter}.jpg',
                bbox_inches='tight', dpi=150)
    plt.rcParams["figure.figsize"] = (5, 5)
    sc.pl.umap(data, color=['louvain'], legend_loc='on data', s=10, show=False,title='stLearn')
...
#     #
#     # eval.Spatialeval(os.path.join(f"./outputss/{category}/", f"{category}_index.csv"),
#     #                  X, y, X.shape[1], dav, cal, sil, sdbw, table, parameter)
#     #
#     print(data.isbacked)
#     if not os.path.exists(f'./h5ad/{category}'):
#         os.makedirs(f'./h5ad/{category}')
#     data.filename = f'./h5ad/{category}/final_{category}_{parameter}.h5ad'
#     print(data.isbacked)
